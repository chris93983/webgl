...
const hqx = (img, scale) => {
    ...
    let origCanvas, origCtx, scaledCanvas, origPixels;
    ...
    if (scale === 2)
        ...
        dest = hq3x(img.width, img.height);
    else if (scale === 4)
        dest = hq4x(img.width, img.height);
...
const hq2x = (width, height) => {
    const dest = new Array(width * height * 4);
    let prevline, nextline, w = [], 
    //dpL = width * 2, optimized
    dpL = width << 1, dp = 0, sp = 0;
...
const hq3x = (width, height) => {
    const dest = new Array(width * height * 9);
    let prevline, nextline, w = [], dpL = width * 3, dp = 0, sp = 0;
    // internal to local optimization
    let YUV1, YUV2;
    //   +----+----+----+
    //   |	|	|	|
